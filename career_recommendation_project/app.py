...
# Sidebar navigation
page = st.sidebar.radio("Navigation", ["Profile Info", "Numerical Quiz", "Logical Quiz", "English Quiz", "Personality"])
...
if page == "Profile Info":
    st.title("Career Recommendation: Profile Information")
    for col in categorical_cols:
        st.session_state.inputs[col] = st.selectbox(f"Select {col}", encoders[col].classes_, key=col)

# Page 2: Numerical Quiz
elif page == "Numerical Quiz":
    st.title("Numerical Ability (10 Questions)")
    questions = [
    ...
    for i, (q, opts, ans) in enumerate(questions):
        user_ans = st.radio(f"Q{i+1}: {q}", opts, key=f"num_{i}")
        if user_ans == ans:
            score += 1
    st.session_state.inputs["Numerical"] = score

# Page 3: Logical Quiz
elif page == "Logical Quiz":
    st.title("Logical Reasoning (10 Questions)")
    questions = [
    ...
    score = 0
    for i, (q, opts, ans) in enumerate(questions):
        user_ans = st.radio(f"Q{i+1}: {q}", opts, key=f"log_{i}")
        if user_ans == ans:
            score += 1
    st.session_state.inputs["Logical"] = score

# Page 4: English Quiz
elif page == "English Quiz":
    st.title("English Grammar (10 Questions)")
    questions = [
    ...
    score = 0
    for i, (q, opts, ans) in enumerate(questions):
        user_ans = st.radio(f"Q{i+1}: {q}", opts, key=f"eng_{i}")
        if user_ans == ans:
            score += 1
    st.session_state.inputs["English"] = score

# Page 5: Personality + Result
elif page == "Personality":
    st.title("Personality Assessment (10 Questions)")
    questions = [
    ...
    for i, q in enumerate(questions):
        score += st.slider(f"{i+1}. {q}", 1, 5, 3, key=f"personality_{i}")
    st.session_state.inputs["Personality"] = score
